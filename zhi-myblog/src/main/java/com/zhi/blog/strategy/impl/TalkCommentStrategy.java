...
import static com.zhi.common.constant.blog.CommonConst.TALK_TYPE_VALUE;
...
public class TalkCommentStrategy implements DeleteType {
    ...
    public void operate(Collection<Long> ids) {
        ids.forEach( i ->{
            List<Comment> comments = commentMapper.selectList(new LambdaQueryWrapper<Comment>().eq(Comment::getType, TALK_TYPE_VALUE).eq(Comment::getTopicId,i));
